...
- Keep the global header geometry identical across Home, Foundation, Components, Patterns, and About. Overview routes must not introduce a separate stretched header grid or larger horizontal page gutters.
- Every preview theme toggle must work independently of the global colour scheme. A light preview inside global dark mode must use an explicit light token scope, and a dark preview inside global light mode must use an explicit dark token scope.
