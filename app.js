...
function bindExample(root, code) {
  ...
  syncExampleThemeToggle(root);
  root.querySelector('[data-invert]').addEventListener('click', () => {
    const dark = !exampleIsDark(root);
    root.classList.toggle('fd-theme-dark', dark);
    root.classList.toggle('fd-theme-light', !dark);
    syncExampleThemeToggle(root);
  });
...
function exampleIsDark(root) {
  if (root.classList.contains('fd-theme-light')) return false;
  if (root.classList.contains('fd-theme-dark')) return true;
  return document.documentElement.classList.contains('fd-theme-dark');
}

function syncExampleThemeToggle(root) {
  const dark = exampleIsDark(root);
  const button = root.querySelector('[data-invert]');
  button?.querySelector('use')?.setAttribute('href', `#${dark ? 'sun' : 'moon'}`);
  button?.setAttribute('aria-label', dark ? 'Use light preview' : 'Use dark preview');
}
...
themeToggle.addEventListener('click', (event) => {
  ...
  document.querySelectorAll('[data-example]:not(.fd-theme-light):not(.fd-theme-dark)').forEach(syncExampleThemeToggle);
